...
### 방법 A — npm (권장, macOS · Windows 공통)

**Node.js ≥ 16** 이 있으면 한 줄로 설치됩니다. OS 에 맞는 바이너리를 GitHub Releases 에서
자동으로 내려받고, 자동시작까지 등록합니다.

```bash
npm install -g oh-my-flunky-daemon
oh-my-flunky-daemon --install    # 자동시작 서비스 등록
```

- macOS: `launchd` 에 등록 (로그: `/tmp/oh-my-flunky-daemon.log`)
- Windows: 작업 스케줄러에 등록 (로그인 시 자동 실행)
- 설치 시 실행 파일이 앱 지원 폴더로 **복사**되므로, 이후 npm 패키지를 지워도 데몬은 계속 동작합니다.

**업데이트:**

```bash
npm update -g oh-my-flunky-daemon
oh-my-flunky-daemon --install    # 새 바이너리를 서비스에 반영
```
...
> npm 으로 설치했다면 바이너리 경로 대신 `oh-my-flunky-daemon` 명령을 그대로 쓰면 됩니다:
...
설치된 실행 파일은 `<앱 지원 폴더>/oh-my-flunky/bin/` 으로 복사되므로, 설치 후에는 내려받은 원본 파일을 삭제해도 됩니다.
...
- **macOS 보안 경고**: 위 1번의 `xattr` 명령 또는 **우클릭 → 열기** 로 허용하세요.
...
- **npm 사용자**: `npm update -g oh-my-flunky-daemon && oh-my-flunky-daemon --install`.
- **직접 다운로드 사용자**: 위 다운로드 링크에서 최신 파일을 다시 받아 한 번 실행하면 갱신됩니다.
